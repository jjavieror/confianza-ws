package co.com.ws.confianza.dao.external;
 ...
 * @version 1.0
 */
public abstract class AbstractJdbcOracleRepository extends BaseJdbcTemplateOracleDAO {

	private static final Logger LOGGER = LoggerFactory.getLogger(AbstractJdbcOracleRepository.class);
	...
	public AbstractJdbcOracleRepository(String beanQueries) {
	...
	public <T> T findObjectBean(String nameQuery, MapSqlParameterSource mapParameters, RowMapper<T> rowMapper) {
		...
		if (getQueryMap() != null && getQueryMap().get(nameQuery) != null) {
			...
			try {
				System.out.println(String.format(getQueryMap().get(nameQuery)));
				StringBuilder query = new StringBuilder(String.format(getQueryMap().get(nameQuery)));
				System.out.println(query.toString() + mapParameters);
